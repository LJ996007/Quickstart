...
4. 如需使用全局右键拖拽呼出，请在菜单栏菜单中选择“启用右键拖拽权限”，并在系统设置中允许辅助功能 / 输入监控权限。
...
除了点击托盘图标或菜单栏入口，Windows 和 macOS 版都支持全局鼠标手势呼出：

- 按住鼠标右键向右拖动，可以快速唤出主弹窗。
- 松开时可以直接选中并执行目标条目。
- macOS 首次使用需要授权辅助功能 / 输入监控权限；未授权时仍可通过 Dock 和菜单栏正常打开。
...
- Dock 图标、菜单栏常驻与搜索弹窗
- 扁平圆角无边框主界面、设置窗口和编辑窗口
- 左侧文件 / 网页 / 文本类型标签，右侧当前类型分组标签
- 全局右键向右拖拽呼出主界面并可松开执行条目
- 文件夹、文件、网页、文本四类条目
- 收藏新增、编辑、删除
- 中文搜索与拼音首字母搜索
- 文件/文件夹拖放添加
