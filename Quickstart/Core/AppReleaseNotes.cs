...
internal static class AppReleaseNotes
{
    ...
    [
        new(
            ...
            "2026-09-13",
            [
                "显示环境自适应：切换屏幕分辨率、热插拔显示器或改变缩放比例时，界面会实时重新适配，无需重启程序（新增显示环境监听；弹窗尺寸按目标显示器 DPI 计算；窗口越界自动夹回工作区）。",
                "修复系统缩放变化后界面尺寸错位（高分屏上内容溢出、错位或被裁切）：窗口 DPI 一律以所在显示器的当前 DPI 为准。隐藏窗口在缩放变化后收不到 DPI 通知、会一直停在旧值，导致窗体尺寸与内部控件口径不一致。",
                "右滑圆环轮手势跟手性大幅改善：命中判定改为连续径向分环（消除环间重叠与空档）、新增扇区切换迟滞、列表按最近行中心命中并覆盖行间距。",
                "圆环轮性能与交互细节：悬停高亮改为每帧叠加绘制，不再每滑过一个扇区就重建整窗位图；手势模式下列表支持自动滚动，超过 8 行的条目不松手也能选到；同一标签内滑回分类环不再重置已选分组。"
            ]),
        new(
            "1.0.20",
            "2026-08-20",
            [
                "右滑圆环轮：分组外环增加「全部」，并放在焦点标签中线最近扇区，径向最短滑轨先到「全部」；分组名单字正立沿弧排布，避免水平撑出环外盖住列表。",
                "右滑圆环轮：中心松手固定后，点击标签会锁定该标签及其分组显示；鼠标移过其它标签不再替换内容，按住右滑时仍保留原有悬停切换逻辑。"
            ]),
        new(
            "1.0.19",
            "2026-08-04",
